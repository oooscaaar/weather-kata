...
class Forecast {
    async predict(city, datetime, wind) {
        ...
        if (datetime < datetime2) {
            ...
            const { latitude, longitude } = await this.getGeolocation(city);

            // Find the predictions for the location
            const {results: {daily}} = await this.getPrediction(latitude, longitude);
            console.log(daily)
            for (let i = 0; i < 7; i++) {
                // When the date is the expected
                if (daily.time[i] === datetime.toISOString().slice(0, 10)) {
                    if (wind) {
                        return daily['windspeed_10m_max'][i];
                    } else {
